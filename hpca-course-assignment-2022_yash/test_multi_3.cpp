            ...
            for(int j=0; j<N/4; j+=64){
                for(int iter=0; iter<64; iter+=2){
                    for(int rowA=0; rowA<64; rowA+=2){
                        ...
                        int rowC = ((i*N)+rowA) >> 1; //((arg->rowS)+(i*N)+rowA) >> 1;
                        ...
                        for(int colB=0; colB<64; colB+=8){
                            ...
                            int colC = ((k*N)+colB)>>1; //(arg->colS+(j*N))
                            //int indexC = arg->outS + (rowIndex + colC)+((i*(N>>1))+j);
                            int indexC = arg->outS+(i*(N)+j)+(rowA*(N>>1)+colB>>1);
                            //cout << indexC << " ";
                            arg->output[indexC] += sum[0] + sum[1];
                            arg->output[indexC+1] += sum[2] + sum[3];
                            arg->output[indexC+2] += sum[4] + sum[5];
                            arg->output[indexC+3] += sum[6] + sum[7];
